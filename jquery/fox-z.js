(function( window , undefined ){
	...
	*/
	function fox( html ){
		return new fox.fn.init( html );
	}

	var push = [].push;

	fox.fn = fox.prototype = {
		...
		init : function( html ){
			...
			if( typeof html === 'string' ){
				...
				if( /^</.test( html ) ){
					...
					push.apply( this , fox.select( html ) );
					this.selector = html ;
				}
			}
		...
		//遍历DOM元素
		each : function ( func ){
			return fox.each ( this , func );
		},

		//遍历DOM元素
		map : function ( func ){
			return fox.map ( this , func );
		}

	}

	//让init继承fox函数上的方法
	fox.fn.init.prototype = fox.fn;

	fox.extend = fox.fn.extend = function( obj ){
		for( var k in obj ){
			this[k] = obj[k];
		}
	} 
	...
	*/
	...
	fox.fn.extend({
		...
		animate : function( prop , dur , easing , fn ){
			...
			var startX = parseInt ( fox.getStyle( this[0] , 'left' ) );
			var startY = parseInt( fox.getStyle( this[0] , 'top' ) );
			var start = +new Date();
			var that = this;
			var isOver = false;
			this.intervalId = setInterval(function(){
				...
				that[0].style.left = startX +
				 fox.Easing[ easing ](  null , t , startX , parseInt( prop['left'] ) , dur ) + 'px';
				that[0].style.top = startY +
				 fox.Easing[ easing ](  null , t , startY , parseInt( prop['top'] ) , dur ) + 'px';
	...
	*事件对象模块
	*/
	fox.each( ("blur focus focusin focusout load resize scroll unload click dblclick " +
  			   "mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave " +
               "change select submit keydown keypress keyup error contextmenu").split(" "), 
	function( i , v ){
		fox.fn[ v ] = function( fn ){
			this.on( v , fn );
			return this;
		};
	});

	window.fox = window.F = fox ;
